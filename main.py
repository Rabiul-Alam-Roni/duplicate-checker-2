...
def doi_exists(session, doi):
    ...
    for article in articles:
        if fuzz.ratio(normalize_doi(article.doi), normalized_doi) > 90:
            return True
    return False
...
def check_article(doi: str = Form(...), title: str = Form(""), url: str = Form(""),
                  hardness: bool = Form(False), whc: bool = Form(False), tags: str = Form("")):
    session = Session()
...
def export_articles():
    ...
    articles = session.query(Article).all()
    df = pd.DataFrame(
        [(a.doi, a.title, 'Yes' if a.hardness else 'No', 'Yes' if a.whc else 'No', a.tags)
         for a in articles],
